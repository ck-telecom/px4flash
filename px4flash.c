...
open_uart(const char *uart_name, int baud)
{
	...
	/* open uart*/
	_boot_fd = open(uart_name, O_RDWR | O_NONBLOCK | O_NOCTTY);
...
static int parse_options(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "b:f:dn:")) != -1) {
		switch (c) {
		case 'b':
...
/*
	for (c = optind; c < argc; ++c) {
		if (dev_name) {
			LOGE("ERROR: Invalid parameter specified!\n");
			return 1;
		}
		dev_name = argv[c];
	}
...
main(int argc, char *argv[])
{
	...
	if (ret)
		exit(1);
LOGI("fuck u!!!\n");
	ret = autopilot_upgrade_start(dev_name, baud, fn);
